...
function diffTokenLists(base, update) {
  const indexedBase = base.reduce(function(memo, tokenInfo) {
  ...
  const newListUpdates = update.reduce(
    function(memo, tokenInfo) {
      let _indexedBase$tokenInf;

      const baseToken =
        (_indexedBase$tokenInf = indexedBase[tokenInfo.chainId]) == null
          ? void 0
          : _indexedBase$tokenInf[tokenInfo.address];
      ...
      if (!memo.index[tokenInfo.chainId]) {
        ...
        memo.index[tokenInfo.chainId] =
          ((_memo$index$tokenInfo = {}),
          (_memo$index$tokenInfo[tokenInfo.address] = true),
          _memo$index$tokenInfo);
      } else {
  ...
  const removed = base.reduce(function(list, curr) {
    if (
